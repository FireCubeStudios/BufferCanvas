...
public class ZIndexCPUBufferCanvas {
    private int WIDTH = 1280;
    private int HEIGHT = 720;
    private int DEPTH = 3;
    private int[] BUFFER = new int[WIDTH * HEIGHT * DEPTH];
    ...
    private Scene scene;
    public ZIndexCPUBufferCanvas(String filename, Stage primaryStage) {
    ...
    void Setup(){
        ...
        BUFFER = new int[WIDTH * HEIGHT * DEPTH];
    ...
    private void Draw()
    {
      ...
      /*  IntStream.range(0, BUFFER.length).parallel().forEach(ii -> {
        ...
        IntStream.range(0, mapData.ways.size()).parallel().forEach(ii -> {
            ...
            if(way.coords[0] == way.coords[way.coords.length - 2] && way.coords[1] == way.coords[way.coords.length - 1])
            {
                ...
                for (int i = 0; i < way.coords.length - 2; i += 2) {
                    ...
                    DrawLine(x, y, x2, y2, 2, toARGB(Color.BLACK));
                }
            }
        });
        ...
        currentBuffer.setPixels(getTopLayer());
    }
    public void reset() {
        ForkJoinPool forkJoinPool = new ForkJoinPool();
        forkJoinPool.submit(() ->
                Arrays.parallelSetAll(BUFFER, i -> {
                    int x = (i % WIDTH) % WIDTH;
                    int y = (i / WIDTH) % HEIGHT;
                    int z = (i / (WIDTH * HEIGHT)) % DEPTH;
                    if (z == 0) {
                        return toARGB(Color.LIGHTBLUE);
                    } else {
                        return 0;
                    }
                })
    ...
    public void oldreset() {
        for (int x = 0; x < WIDTH; x++) {
            for (int y = 0; y < HEIGHT; y++) {
                BUFFER[x + y * WIDTH] = toARGB(Color.LIGHTBLUE);
                for (int z = 1; z < DEPTH; z++) {
                    BUFFER[x + y * WIDTH + z * WIDTH * HEIGHT] = 0;
                }
            }
        }
    }
    // Method to get the topmost layer of the buffer
    public int[] getTopLayer() {
        int[] topLayer = new int[WIDTH * HEIGHT];
        ForkJoinPool forkJoinPool = new ForkJoinPool();
        forkJoinPool.submit(() ->
                Arrays.parallelSetAll(topLayer, i -> {
                    int x = i % WIDTH;
                    int y = i / WIDTH;
                    int topPixel = 0;
    ...
    void DrawPolygon(int[] vertices, int c){
        ...
        for (int y = minY; y <= maxY; y++) {
            ...
            for (int i = 0; i < count; i += 2) {
                ...
                DrawLine(x1, y, x2, y, 0, c);
            }
        }
    }

    void DrawLine(int x1, int y1, int x2, int y2, int z, int c)
    {
        if((x1 > 0 && x1 < WIDTH && y1 > 0 && y1 < HEIGHT) || (x2 > 0 && 2 < WIDTH && y2 > 0 && y2 < HEIGHT)){
            ...
            while (x != x2 || y != y2) {
                if (x > 0 && y > 0 && x < WIDTH && y < HEIGHT) {
                    int position = x + y * WIDTH + z * WIDTH * HEIGHT;

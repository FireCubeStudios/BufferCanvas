...
public class FIXED2POLYGONKERNEL extends Kernel {
    public int[] buffer;
    ...
    public int[] ScanHorizontalBuffer; // Stores horizontal scanline filling results
    public int[] ScanVerticalBuffer; // Stores vertical scanline filling results
    ...
    public FIXED2POLYGONKERNEL(int[] buffer, int[] BACKGROUND, int width, int height) {
        this.buffer = buffer;
        this.BACKGROUND = BACKGROUND;
        this.lines = new int[10 * 2];
        this.SCANPOLYGON = new int[buffer.length];
        this.ScanHorizontalBuffer = new int[buffer.length];
        this.ScanVerticalBuffer = new int[buffer.length];
        this.SCANCOLOURS = new int[width * 10];
        this.mode[0] = 0;
        this.size[0] = width;
        this.size[1] = height;
        ...
        put(this.buffer);
        put(this.ScanHorizontalBuffer);
        put(this.ScanVerticalBuffer);
        put(this.lines);
        put(this.BACKGROUND);
    ...
    public void run() {
        ...
        // Scanline fill polygon
            ...
            while(row < size[1])
            {
                ...
                {
                    startTracking = true;
                    ScanVerticalBuffer[position] = SCANPOLYGON[position];
                ...
                else if((SCANPOLYGON[position] != SCANCOLOURS[colourIndex] || SCANPOLYGON[position] == 0) && !startFilling && startTracking && !endTracking)
                {
                    startFilling = true;
                    startTracking = false;
                    ScanVerticalBuffer[position] = SCANCOLOURS[colourIndex];
                }
        ...
        else if(mode[0] == 3)
        {
            ...
            while(col < size[0])
            {
                ...
                {
                    startTracking = true;
                    ScanHorizontalBuffer[position] = SCANPOLYGON[position];
                    colourIndex = colourIndex + 1;
                    SCANCOLOURS[colourIndex] = SCANPOLYGON[position];
                }
                // Track until end of start border found
                else if((SCANPOLYGON[position] != SCANCOLOURS[colourIndex] || SCANPOLYGON[position] == 0) && !startFilling && startTracking && !endTracking)
                {
                    startFilling = true;
                    startTracking = false;
                    ScanHorizontalBuffer[position] = SCANCOLOURS[colourIndex];
                }
                // start of end border of current polygon encountered
                else if(colourIndex > (i * 10) && SCANPOLYGON[position] == SCANCOLOURS[colourIndex] && startFilling && !startTracking && !endTracking)
                {
        ...
        {
            int h = ScanHorizontalBuffer[i];
            int v = ScanVerticalBuffer[i];
            if(h != 0 && v != 0 && h == v)
            {
                buffer[i] = h;
            }
        ...
        // Draw background to clear canvas and scanned polygons
            ...
            if (i < buffer.length) {
                ...
                SCANPOLYGON[i] = 0;
                ScanHorizontalBuffer[i] = 0;
                ScanVerticalBuffer[i] = 0;
            }
        }
    }
}
